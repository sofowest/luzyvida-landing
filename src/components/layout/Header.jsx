...
function Header() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      setScrolled(window.scrollY > 50); // Cambia 50 por la cantidad de scroll deseada
    };
  ...
  return (
    <header className={`bg-[var(--background-opacity-color)] transition-shadow ${scrolled ? 'shadow-md' : 'shadow-none'} backdrop-blur-sm text-white fixed top-0 w-[100vw] h-18 flex m-0 p-0 flex-col z-50
